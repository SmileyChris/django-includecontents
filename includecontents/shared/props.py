...
def _parse_prop_token(token: str) -> tuple[str, Any]:
    if "=" not in token:
        ...
        # Handle typed syntax like "title:str" or "title?:str" - extract just the prop name
        if ":" in name:
            name = name.split(":", 1)[0].strip()

        # Handle optional syntax like "title?" - remove the question mark
        if name.endswith("?"):
            name = name[:-1].strip()
    ...
    # Handle typed syntax like "title:str=default" or "title?:str=default" - extract just the prop name
    if ":" in name:
        name = name.split(":", 1)[0].strip()
    ...
    try:
        ...
        if ',' in raw and ' ' not in raw and not any(c in raw for c in ['(', ')', '[', ']', '{', '}', '"', "'"]):
            ...
            value = raw
        # If it contains spaces or special chars, it's likely meant to be a string value
        # This happens when shlex strips quotes from quoted values
        elif ' ' in raw or any(c in raw for c in ['(', ')', '[', ']', '{', '}', '&', '|', ':', ';', '!', '?']):
            # Treat as string value (quotes were stripped by tokenizer)
            value = raw
        else:
