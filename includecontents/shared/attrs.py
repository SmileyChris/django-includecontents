...
class BaseAttrs(MutableMapping[str, Any]):
    ...
    def update(self, other) -> None:
        """
        Update this attrs object with another attrs object or dict.

        For BaseAttrs objects, merges the internal structure properly.
        For dicts, uses the standard MutableMapping update behavior.
        """
        if isinstance(other, BaseAttrs):
            # Merge BaseAttrs objects by copying internal structures

            # Merge regular attributes
            self._attrs.update(other._attrs)

            # Merge conditional modifiers
            for key, modifiers in other._conditional_modifiers.items():
                if key in self._conditional_modifiers:
                    self._conditional_modifiers[key].update(modifiers)
                else:
                    self._conditional_modifiers[key] = modifiers.copy()

            # Merge prepended classes
            for key, parts in other._prepended_classes.items():
                if key in self._prepended_classes:
                    self._prepended_classes[key].update(parts)
                else:
                    self._prepended_classes[key] = parts.copy()

            # Merge nested attrs
            for key, nested in other._nested_attrs.items():
                if key in self._nested_attrs:
                    self._nested_attrs[key].update(nested)
                else:
                    self._nested_attrs[key] = nested
        else:
            # Use default MutableMapping behavior for dicts
            super().update(other)
