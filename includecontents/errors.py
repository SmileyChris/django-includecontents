...
def enhance_error(exc: Exception, note: str) -> Exception:
    ...
    Uses Exception.add_note() if available (Python 3.11+), otherwise
    modifies the exception args to include the note.
    ...
    if hasattr(exc, 'add_note'):
        ...
        # Fallback for older Python versions - append to args
        if exc.args:
