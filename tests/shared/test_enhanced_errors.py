...
from includecontents.django.errors import (
    enhance_error,
    enhance_validation_error,
    enhance_coercion_error,
)
...
class TestErrorEnhancement:
    ...
    def test_enhance_error_fallback_behavior(self):
        ...
        with patch("includecontents.django.errors.hasattr", return_value=False):
            enhanced = enhance_error(exc, "Additional context")
    ...
    def test_enhance_coercion_error(self):
        ...
        enhanced = enhance_coercion_error(
            exc, field_name="age", value="abc", expected_type=int
        )
...
class TestEnhancedValidationErrors:
    ...
    def test_validation_error_includes_component_context(self):
        ...
        with pytest.raises(TemplateSyntaxError) as exc_info:
            validate_props(TestCardProps, {"email": "invalid-email", "age": 25})
    ...
    def test_type_coercion_error_context(self):
        ...
        with pytest.raises(TemplateSyntaxError) as exc_info:
            validate_props(
                TypedProps, {"count": "not-a-number", "price": 10.5, "active": True}
            )
