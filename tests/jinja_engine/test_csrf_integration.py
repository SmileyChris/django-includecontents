...
def test_csrf_token_available_in_components() -> None:
    ...
    template = env.from_string('{% includecontents "form" action="/submit" %}Fields{% endincludecontents %}')
...
def test_csrf_token_preserved_in_isolated_context() -> None:
    ...
    template = env.from_string('{% includecontents "secure-form" title="Login" %}Username: <input name="username">{% endincludecontents %}')
...
def test_no_csrf_token_when_not_provided() -> None:
    ...
    template = env.from_string('{% includecontents "optional-csrf" %}Content{% endincludecontents %}')
...
def test_csrf_input_vs_csrf_token() -> None:
    ...
    template = env.from_string('{% includecontents "dual-csrf" %}Test{% endincludecontents %}')
    ...
    assert 'name="csrfmiddlewaretoken" value="token-value-999"' in rendered  # From csrf_input
