...
class ComplexNestedComponentsTest(TestCase):
    ...
    def test_processor_data_isolation_between_components(self):
        ...
        modifier_lines = [line for line in content_lines if 'modifier-component' in line]
    ...
    def test_csrf_token_preservation_in_nested_components(self):
        ...
        # Note: Django generates its own CSRF token via middleware, so we check for that
    ...
    def test_multiple_processor_data_sources_merge_correctly(self):
        ...
        content = render_to_string(
            'test_multi_processor_merge.html',
            context={},
            request=self.request
        )
    ...
    def test_processor_data_with_component_slots(self):
        ...
        # Note: slot_data from context should be available in default content, but
        # in component isolation mode, parent context variables may not be passed through
        # Check if at least the default slot renders without errors
