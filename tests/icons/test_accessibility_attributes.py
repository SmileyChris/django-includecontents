...
def test_aria_attributes_on_svg_element(mock_fetch):
    ...
    template_str = '<icon:home role="img" aria-label="Go to homepage" class="nav-icon" />'
...
def test_aria_attributes_on_use_element(mock_fetch):
    ...
    template_str = '<icon:home class="nav-icon" use.role="img" use.aria-label="Go to homepage" />'
    template = CustomTemplate(template_str)
    result = template.render(Context())
...
def test_mixed_attribute_placement(mock_fetch):
    ...
    mock_fetch.side_effect = lambda prefix, names, api: (
        {'star': {'body': '<path d="star"/>', 'viewBox': '0 0 24 24'}} if prefix == 'mdi' else {}
    )
...
def test_meaningful_icon_pattern(mock_fetch):
    ...
    mock_fetch.side_effect = lambda prefix, names, api: (
        {'content-save': {'body': '<path d="save"/>', 'viewBox': '0 0 24 24'}} if prefix == 'mdi' else {}
    )
    
    template_str = '<icon:save role="img" aria-label="Save document" class="action-icon" />'
    template = CustomTemplate(template_str)
    result = template.render(Context())
...
def test_accessibility_best_practices_documentation():
    ...
    icon_button_example = '<button><icon:edit role="img" aria-label="Edit item" /></button>'
